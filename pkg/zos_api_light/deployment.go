...
// rootfs subvolume (see pkg/primitives/vm-light/utils.go). We reuse it here to
// transfer/restore the user's rootfs changes.
const rootfsVolumePrefix = "rootfs:"

// waitWorkloadTimeout bounds how long prepare waits for an async workload
// (zmount) to be provisioned before pulling its data.
const (
...
func (g *ZosAPI) runDownloads(twin uint32, contractID uint64, jobs []transferJob, start bool) {
	...
	for _, j := range jobs {
		...
		default: // zmachine (light) rootfs; VolumeDownload creates the volume
